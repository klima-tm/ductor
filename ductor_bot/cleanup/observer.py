...
class CleanupObserver(BaseObserver):
    """Runs daily cleanup for transport, generated, and monitored media files.
    ...
    async def _execute(self) -> None:
        ...
        targets = [
            ...
            (self._paths.instagram_files_dir, self._cfg.media_files_days),
        ]
        results = await asyncio.to_thread(_run_cleanup, targets)

        if any(results):
            logger.info(
                "Cleanup complete: telegram=%d, output=%d, api=%d, matrix=%d, instagram=%d",
                results[0],
                results[1],
                results[2],
                results[3],
